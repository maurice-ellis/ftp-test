...
export class FtpTreeDataProvider implements vscode.TreeDataProvider<FtpNode>, vscode.TextDocumentContentProvider {
    ...
    public refresh(element: FtpNode): any {
...
export class FtpExplorer {
    ...
    private treeDataProvider: FtpTreeDataProvider;
    ...
    constructor(context: vscode.ExtensionContext) {
        ...
        this.treeDataProvider = new FtpTreeDataProvider(ftpModel);
        context.subscriptions.push(vscode.workspace.registerTextDocumentContentProvider('ftp', this.treeDataProvider));

        this.ftpViewer = vscode.window.createTreeView('ftpExplorer', { treeDataProvider: this.treeDataProvider });

        vscode.commands.registerCommand('ftpExplorer.refresh', (item) => this.refresh(item));
        vscode.commands.registerCommand('ftpExplorer.refreshParent', (item) => this.refreshParent(item));
    ...
    /** 
     * Refresh element tree view. 
     * @param element Specific element to refresh;
     */
    refresh(element: FtpNode) {
        console.log("REFRESH ELEMENT, WORKS");
        console.log("ELEMENT", element);
        this.treeDataProvider.refresh(element);
    }

    /** 
     * Refresh parent element tree view. 
     * @param element Specific element to refresh;
     */
    refreshParent(element: FtpNode) {
        console.log("REFRESH PARENT OF ELEMENT, DOES NOT WORK");
        var parent = this.treeDataProvider.getParent(element);
        console.log("ELEMENT", element);
